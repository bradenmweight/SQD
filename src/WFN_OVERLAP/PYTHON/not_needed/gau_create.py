...
class gau_create():
    ...
    def wrt_gau_input(self, flag="default"):
        ...
        # write link0
        link0 = t['link0']
        print("%%chk=%s" % link0['chk'], file=fp) 
        print("%%mem=%s" % link0['mem'], file=fp)
        print("%%nproc=%s" % link0['nproc'], file=fp)
        print("%%nproclinda=%s" % link0['nproclinda'], file=fp)
        # write routine       
        print("%s\n" % t['routine']['content'], file=fp)
        print("%s\n" % t['title'], file=fp)
        # charge & spin
        print("%-4d%-4d" % (t['charge'], t['spin']), file=fp)  
        # molecular geometry
        geom = t['mol'] 
        atoms = geom['atoms']
        natom = geom['natom']
        ...
        for i in range(natom):
            ...
            if "frg" in list(record.keys()):
                frg = record['frg']
                if frg == "":
                    print("%-10s%12.7f%12.7f%12.7f" % (atomname, \
                            float(coord[0]), float(coord[1]), float(coord[2])), file=fp)
                else:
                    print("%-10s%12.7f%12.7f%12.7f%5d" % (atomname, \
                            float(coord[0]), float(coord[1]), float(coord[2]), frg), file=fp)
            else:
                print("%-10s%12.7f%12.7f%12.7f" % (atomname, \
                            float(coord[0]), float(coord[1]), float(coord[2])), file=fp)                
        print("\n", end=' ', file=fp)
        if t['tail'] != "":
            print("%s" % t['tail'], file=fp)

        # write extra job
        if 'force_content' in list(t['routine'].keys()):
            ...
            print("%%nproclinda=%s" % link0['nproclinda'], file=fp)
            # write routine
            print("%s\n" % force_content, file=fp)
        
        if 'es_content' in list(t['routine'].keys()):
