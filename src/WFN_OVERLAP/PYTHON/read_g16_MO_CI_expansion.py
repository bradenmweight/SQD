...
# THIS WORKS FOR G16 TD CALCULATION. NO SPECIFIC INPUT REQUIRED.
...
def dump_json(filename, obj, encode='utf-8'):
    ...
    my_str = json.dumps(obj, indent=2)#, encoding=encode, indent=2)
    fp.write(my_str)
    fp.close()
...
def load_json(filename, encode='utf-8'): 
    ...
    # 
    #  'geometry.chk & geometry.out is required.
    #  
...
class gau_rwf_parser():
    ...
    def __init__(self, config = {}):
        ...
        """
        self.config = config
        self.files = {'chk': "geometry.chk", 'mo': 'geometry.out'}
        self.dim = {}  
        self.ci = {}
        self.mo = {}     
        ...
        self.is_do_ci_assignment = "no"   # default: do not do cis-casida
        self.ci_assign_factor = [0.0, 0.0]
        if 'is_do_ci_assignment' in list(config.keys()):
            self.is_do_ci_assignment = config['is_do_ci_assignment']
        if 'ci_assignment_factor' in list(config.keys()):
            self.ci_assign_factor = [float(f) for f in config['ci_assign_factor'].split()]
                    
        return        


# -------------------------------------------------------------------------
#   geometry.out file is required.
...
# ---------------------------------------------------------------------------    
    def __get_dim_info(self):
        ...
        dump_data('dimension.json', self.dim)                
...
# ---------------------------------------------------------------------------  
    ...
    def __read_mo(self):
        ...
        alpha = [[data[i*n_basis+j] for j in range(n_basis)] for i in range(n_basis)]
        # only alpha is used, for the case of close shell only
    ...
    def __wrt_mo(self):
        ...
        # MO_COEFFS = np.zeros(( n_basis, n_basis ))
        # for mo1 in range(n_basis):
        #     for ao1 in range(n_basis):
        #         MO_COEFFS[mo1,ao1] = coeffs[mo1][ao1]
        # np.savetxt( "MO_MAT.dat", MO_COEFFS )
        # plt.imshow( np.abs(MO_COEFFS), origin='lower', cmap="hot_r" )
        # plt.colorbar(pad=0.01)
        # plt.xlabel("MOBasis Index",fontsize=15)
        # plt.ylabel("AO Basis Index",fontsize=15)
        # plt.tight_layout()
        # plt.savefig("MO_MAT.jpg",dpi=600)
...
# ---------------------------------------------------------------------------                
    def __read_ci_td_energy(self):
        ...
        self.ci['energy'] = [0 for i in range(0, n_state)]
        self.ci['moments'] = [[0 for i in range(15)] for j in range(0, n_state)]
    ...
    def __distrib_ci_matrix(self):
        ...
        self.ci['state'] = [{} for i in range(n_state)]  
        self.ci['n_state'] = n_state    
             
        # four condition, X OR X+Y; \EPSION OR NOT
             
        for i_state in range(1, n_state):
            add_alpha = [[0.0 for i in range(nvir_allA)] for j in range(nocc_allA)] 
            sub_alpha = copy.deepcopy(add_alpha)
            add_beta  = [[0.0 for i in range(nvir_allB)] for j in range(nocc_allB)]
            ...
            if ci_type == "X+Y":
                ...
                alpha_coeffs = [[(add_alpha[i][j]+sub_alpha[i][j]) for j in range(nvir_allA)] for i in range(nocc_allA)]
                beta_coeffs = [[(add_beta[i][j]+sub_beta[i][j]) for j in range(nvir_allB)] for i in range(nocc_allB)]
            else:
    ...
    def __do_ci_assignment(self):
        ...
        """
        if self.is_do_ci_assignment != "yes":
             return
        # assignment factor 
        f0, f1 = self.ci_assign_factor
        ...
        for i_state in range(1, n_state):                    
            ...
            for i in range(noccA):
                for j in range(nvirA):
                    ...
                    p1 = math.sqrt(de/omega) * f1
                    p2 = math.sqrt(1/de) * f2
                    alpha_coeffs[i][j] = alpha_coeffs[i][j]  * p1 * p2
    ...
    def __mip_ci_td(self):
        ...
        for i_state in range(1,n_state):
            i_all=0
            ci_info_state = [{} for i in range(n_occ*n_vir)]
            one_state = self.ci['state'][i_state]
            alpha_coeffs = one_state['alpha_coeffs']
            for i_ci_1 in range(n_occ) :
                for i_ci_2 in range(n_vir) :
